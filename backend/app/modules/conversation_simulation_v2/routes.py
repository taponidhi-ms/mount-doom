...
from app.modules.conversation_simulation_v2.models import (
...
# New router for V2
router = APIRouter(prefix="/conversation-simulation-v2", tags=["Conversation Simulation V2"])

@router.post("/simulate", response_model=ConversationSimulationResponse)
async def simulate_conversation_v2(request: ConversationSimulationRequest):
    """
    Simulate a conversation between C1 and C2 agents using V2 Python-driven workflow.
    """
    MAX_TURNS = 15
    logger.info("Received conversation simulation V2 request", 
               max_turns=MAX_TURNS,
    ...
    try:
        ...
        # Use conversation simulation v2 service
        ...
        logger.info("Simulation V2 completed, preparing for database save", 
        ...
        logger.error("Error in conversation simulation V2", error=str(e), exc_info=True)
        raise
...
async def browse_conversation_simulations(
    ...
    """
    Browse conversation simulation V2 records with pagination and ordering.
    """
    ...
    try:
        result = await cosmos_db_service.browse_container(
            container_name=cosmos_db_service.CONVERSATION_SIMULATION_V2_CONTAINER,
            page=page,
            page_size=page_size,
            order_by=order_by,
            order_direction=order_direction
        ...
        logger.info("Returning browse results V2", total_count=result["total_count"])
        return result

    except Exception as e:
        logger.error("Error browsing conversation simulations V2", error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error browsing conversation simulations V2: {str(e)}")

@router.post("/delete")
async def delete_conversation_simulations(conversation_ids: list[str]):
    """
    Delete conversation simulation V2 records by their IDs.
    """
    logger.info("Received delete request V2", count=len(conversation_ids))
    ...
    try:
        container = await cosmos_db_service.ensure_container(
            cosmos_db_service.CONVERSATION_SIMULATION_V2_CONTAINER
        )
        ...
        logger.error("Error deleting conversation simulations V2", error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error deleting conversation simulations V2: {str(e)}")


@router.post("/download")
async def download_conversation_simulations_v2(conversation_ids: list[str]):
    """
    Download conversation simulation V2 records as JSON.
    Returns conversations array with full metadata and conversation history.
    """
    logger.info("Received download request V2", count=len(conversation_ids))
    ...
    try:
        ...
        logger.info("Returning download data V2", conversation_count=len(conversations))
        return Response(
            content=json_str,
            media_type="application/json",
            headers={"Content-Disposition": 'attachment; filename="conversation_simulations_v2.json"'}
        )
        
    except Exception as e:
        logger.error("Error downloading conversation simulations V2", error=str(e), exc_info=True)
