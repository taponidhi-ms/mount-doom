...
export default function ConversationSimulationV2Page() {
  ...
  const runBatchSimulation = async () => {
    ...
    for (let i = 0; i < newItems.length; i++) {
      ...
      try {
        const response = await apiClient.simulateConversationV2(
  ...
  const loadHistory = async (page: number = 1, pageSize: number = 10) => {
    ...
    setHistoryError('')
    const response = await apiClient.browseConversationSimulationsV2(page, pageSize)
  ...
  const handleDeleteSelected = async () => {
    ...
    const response = await apiClient.deleteConversationSimulationsV2(conversationIds)
  ...
  const handleDownloadConversations = async () => {
    ...
    const conversationIds = selectedRowKeys.map(key => String(key))
    const response = await apiClient.downloadConversationSimulationsV2(conversationIds)
          ...
          <Card title="Conversation Configuration (V2)">
            <Space direction="vertical" size="middle" style={{ width: '100%' }}>
              ...
              <Button 
                ...
                block
              >
                {loading ? 'Simulating...' : 'Start Simulation V2'}
              </Button>
                ...
                placeholder='[
  {
    ...
    "ConversationSubject": "Internet connection keeps dropping"
              ...
              <Space wrap>
                ...
                >
                  {batchLoading ? 'Running Batch...' : 'Start Batch Simulation V2'}
                </Button>
                {batchLoading && (
                  <Button 
                    danger 
  ...
  return (
    <PageLayout
      ...
      description="Simulate multi-turn conversations v2 with distinct C1/C2 logic."
      showBackButton
    >
      <Tabs 
